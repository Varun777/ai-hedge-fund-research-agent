...
def load_security_master():
    ...
    nasdaq_text = requests.get(
        NASDAQ_LISTED_URL,
        timeout=20
    ).text

    other_text = requests.get(
        OTHER_LISTED_URL,
        timeout=20
    ).text
...
def resolve_company_with_security_master(
    company_name,
    securities
):
...
def get_market_reaction(history):
    ...
    return {
        "one_day_return_percent": round(
            ((last_close - closes.iloc[-2]) / closes.iloc[-2]) * 100,
            2
        ),

        "five_day_return_percent": round(
            ((last_close - closes.iloc[-6]) / closes.iloc[-6]) * 100,
            2
        ),

        "twenty_day_return_percent": round(
            ((last_close - closes.iloc[-21]) / closes.iloc[-21]) * 100,
            2
        )
    }
...
def compute_dynamic_signal_score(
    base_score,
    market_data
):
    ...
    one_day = abs(
        market_data.get("one_day_return_percent") or 0
    )
...
def get_market_data_for_ticker(
    ...
    try:
        ...
        market_data["dynamic_signal_score"] = (
            compute_dynamic_signal_score(
                signal_score,
                market_data
            )
        )
...
def send_email(report):
    ...
    subject = (
        f"Daily Investment Thesis Memo - "
